...
class RestaurantTest {
    ...
    public void is_restaurant_open_should_return_true_if_time_is_between_opening_and_closing_time(){
        LocalTime timeNow = LocalTime.parse("11:30:00");
        Restaurant r = Mockito.spy(new Restaurant("Amelie's cafe","Chennai",openingTime,closingTime));
        Mockito.when(r.getCurrentTime()).thenReturn(timeNow);
        assertTrue(r.isRestaurantOpen());
    }

    @Test
    public void is_restaurant_open_should_return_false_if_time_is_outside_opening_and_closing_time(){
        LocalTime timeNow = LocalTime.parse("09:30:00");
        Restaurant r = Mockito.spy(new Restaurant("Amelie's cafe","Chennai",openingTime,closingTime));;
        Mockito.when(r.getCurrentTime()).thenReturn(timeNow);
